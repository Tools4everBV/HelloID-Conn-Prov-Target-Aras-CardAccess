#################################################
# HelloID-Conn-Prov-Target-Aras-CardAccess-Enable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        'EnableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Enabling Aras-CardAccess account with accountReference: [$($actionContext.References.Account)]"
                $enabledAccount = Invoke-RestMethod @splatEnableParams

                if ($enabledAccount.Result -ne 0) {
                    throw $enabledAccount.message
                }
            } else {
                Write-Information "[DryRun] Enable Aras-CardAccess account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not enable Aras-CardAccess account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditLogMessage = "Could not enable Aras-CardAccess account. Error: $($_.Exception.Message)"
